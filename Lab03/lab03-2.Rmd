...
regressaoLogistica
```
Após observar a regressão logística gerada, pode-se observar que a acurácia é de . Podendo enfatizar a importância da acurácia encontrada pois ela vai dizer a precisão que as observações estão classificadas corretamente.
...
arvoreDecisao
```
O algortmo escolhe a melhor acurácia, e o resultado que obtemos no nosso modelo de árvore foi de cp =  aproximadamente. Esse modelo nos retornou uma acurácia melhor do que a regressão logística, indicando que os valores podem ser melhor classificados.
...
adaboost
```
Após observar o resultado do modelo Adaboost, podemos confirmar que
