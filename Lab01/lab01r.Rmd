...
```{r}
dadosCEAP <- read.csv("/Users/mattheusbrito/git/AD2/Lab01/dados/dadosCEAP.csv")
dadosCEAP$valorGlosa <- as.numeric(sub(",", ".", dadosCEAP$valorGlosa, fixed = TRUE))
dadosLimiteCEAP <- read.csv("/Users/mattheusbrito/git/AD2/Lab01/dados/limiteMensalCEAP.csv")
...
*O gráfico acima mostra os 10 parlamentares que mais gastaram dinheiro da CEAP, sendo assim, pode-se notar que a variância entre os valores é ínfima. Variando apenas 0.08M aproximadamente e destacando o parlamentar Edio Lopes que 1.48M em reais do dinheiro da CEAP.*
...
```
*O parlamentar que menos gastou foi o Camilo Cola, seguido por Marcio Monteiro e Marcelo Almeira. Após a análise desse gráfico podemos afirmar que a diferença entre Camilo e Marcio é significativa, devido ao fato de que por mais que os valores não sejam tão altos comparados ao gráfico anterior, ainda é uma grande diferença olhando pelo lado porporcional.*
...
```{r results=FALSE}
mapa <- readOGR("/Users/mattheusbrito/git/AD2/Lab01/mapa/BRUFE250GC_SIR.shp")
```
...
gastosPartidosPB %>%
  ...
  layout(margin = list(l = 150),
         yaxis = list (title = ""))
```
*No primeiro gráfico, o partido que mais fez uso do dinheiro da CEAP foi o PMDB, que fez uso 4052 vezes. Sendo uma diferença significativa para os outros partidos onde o que mais se aproxima foi o PP que fez uso 2156 vezes que equivale a um pouquinho mais da metade das vezes que o PMDB utilizou.*
...
```
*Observando o gráfico vemos que Eduardo Barbosa foi o parlamentar que ultrapassou mais vezes o limite mensal do seu estado, seguido por Rômulo Gouveia e Nelson Marquezelli, sendo 21, 20 e 19 o número de vezes, respectivamente.*
```{r}
excedemLimite <- dadosCEAP %>%
  mutate(ano = substr(dataEmissao, 1, 4)) %>%
  mutate(mes = substr(dataEmissao, 6, 7)) %>%
...
```
*O estado que mais gasta com passagens aéreas é São Paulo, resultando em um total de R$ 901245 aproximadamente, seguido pelo estado de Amazonas com um total de R$ 638953 aproximadamente*
...
```
*Como esperado o tipo de despesa que gerou mais gasto (em R$) foi o de Emissão de Bilhete Aéreo, dando uma despesa de R$ 45242357 aproximadamente*
